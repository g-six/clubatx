  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (evt && ctx.user?.username) {
        const booking = await bookRecording({
          team: data.team,
    ...
    <>
      ...
      <Dialog open={isOpen} onClose={setIsOpen}>
        <form action="" method="POST" onSubmit={handleSubmit}>
          ...
          <DialogDescription>Please fill in the details for the event.</DialogDescription>
          <DialogBody>
            {/* ... */}
            <Fieldset>
              <FieldGroup>
            ...
            {booking_id ? (
              <Button
                href={`https://pay.clubathletix.com/b/eVq3cv5lJ53dcyt9T9aAw05?${[
                  `locked_prefilled_email=${ctx.user?.username}&client_reference_id=${booking_id}`,
                ].join('&')}`}
                target="_blank"
                rel="noreferrer"
                color="lime"
